...
export default function InvoiceRow({ invoice, onView, onEdit }) {
    ...
    <tr className="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
      ...
      <td className="py-4 px-4">
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
          isSale ? 'bg-primary/10 text-primary border border-primary/20' : 'bg-secondary/10 text-secondary border border-secondary/20'
        }`}>
          {isSale ? 'بيع' : 'شراء'}
        </span>
      </td>
      <td className="py-4 px-4">
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
          isCredit ? 'bg-error/10 text-error border border-error/20' : 'bg-surface-variant text-on-surface border border-white/10'
        }`}>
